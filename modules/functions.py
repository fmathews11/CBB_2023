...
def add_pie_to_dataframe(input_df:pd.DataFrame) -> pd.DataFrame:
  ...
  for player, sub_dict in df.set_index("Player").to_dict(orient = 'index').items():
      ...
      numerator = (sub_dict['PTS'] + sub_dict['FGM'] + sub_dict['FTM'] + sub_dict['FTA'] + sub_dict['DREB'] + (0.5* sub_dict['OREB']))
      numerator += (sub_dict['AST'] + (0.5 * sub_dict['BLK']) - sub_dict['PF'] - sub_dict['TO'])

      denominator = (master_dict['PTS'] + master_dict['FGM'] + master_dict['FTM'] + master_dict['FTA'] + master_dict['DREB'] + (0.5* master_dict['OREB']))
